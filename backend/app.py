...
def create_app() -> Flask:
    ...
    app.config["JWT_EXPIRATION_DELTA"] = int(
        os.getenv("JWT_EXPIRATION_DELTA", "86400")
    )
    ...
    def health_root():
        ...
        return jsonify(
            {
                "status": "ok",
                "message": "Product UX Community Platform API",
            }
        )

    # 헬스 체크 (/api용 – 통합 테스트, 프론트에서 사용)
...
if __name__ == "__main__":
    ...
    port = int(flask_app.config.get("PORT", 8000))
    debug = bool(flask_app.config.get("DEBUG", True))
